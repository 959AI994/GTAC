...
_win_exe = ".exe" if platform.system() == "Windows" else ""


def _resolve_executable(env_var: str, bin_relative: str, which_names: tuple[str, ...]) -> str:
    """Prefer env override, then package bin/, then PATH (Berkeley ABC / aigtoaig)."""
    override = os.environ.get(env_var)
    if override:
        return override
    stem = os.path.join(base_path, bin_relative.replace("/", os.sep))
    for candidate in (stem + _win_exe, stem):
    ...
    for name in which_names:
        ...
        if found:
            return found
    return stem + _win_exe if _win_exe else stem


def _resolve_config_file(env_var: str, bin_relative: str) -> str:
    override = os.environ.get(env_var)
    if override and os.path.isfile(override):
        return override
    return os.path.join(base_path, bin_relative.replace("/", os.sep))


abc_path = _resolve_executable("SACT_ABC_PATH", "bin/abc", ("abc",))
aigtoaig_path = _resolve_executable("SACT_AIGTOAIG_PATH", "bin/aigtoaig", ("aigtoaig",))
abc_rc_path = _resolve_config_file("SACT_ABC_RC_PATH", "bin/abc.rc")
temp_dir = None
...
def compute_tt(root: NodeWithInv | Node, num_inputs=None, input_tt=None, cache=None):
    # Ensure num_inputs is determined early for constant nodes
    if input_tt is None:
        assert num_inputs is not None, "Must provide either input_tt or num_inputs"
    ...
    def compute_tt_rec(root: NodeWithInv) -> bitarray.bitarray:
        ...
        if root is None:
            ...
            if root.var >= 0:
                ...
                # Use input_tt[0] length to ensure consistency
                res = bitarray.util.zeros(len(input_tt[0]))
...
def check_conflict(
    tree_stack: list[NodeWithInv],
    tt,
    input_tt,
    current_tt_conflict=None,
    cache=None,
    tolerance=0.0,
    eta=0.01,
    verbose=0,
    ...
    """
    Conflict check against target truth table `tt` for a set of candidates.

    Notes on parameters:
    - `input_tt` is the truth-table basis for *input variables* (x, ~x, constants) and
      is what compute_tt() expects for leaf evaluation.
    - `initial_tt` (if provided) is treated as the *candidate set* (the `b` vectors).
      In that case, the number of candidates must come from `len(initial_tt)`.
    - `base_input_tt` (optional) is used for compute_tt() fallback when cache misses;
      if None, defaults to `input_tt`.
    - `total_patterns` (optional): denominator for allowed bit/pattern errors. Defaults to
      ``len(tt)``; use when ``tt`` is care-sampled and ``len(tt)`` is the effective column
      count for pattern-level error rate alignment.
    """
    # print(f"tolerance: {tolerance}")
    tt_size = len(tt)
    denom = tt_size if total_patterns is None else int(total_patterns)
    if denom <= 0:
        denom = tt_size
    candidates_tt = initial_tt if initial_tt is not None else input_tt
    num_candidates = len(candidates_tt)
    def is_temp_node(n):
        if n is None: return False
        if hasattr(n, 'var') and n.var == -2: return True
        if hasattr(n, 'node') and hasattr(n.node, 'var') and n.node.var == -2: return True
        return False

    b = [item.copy() for item in candidates_tt]

    if base_input_tt is None:
        base_input_tt = input_tt

    n = [bitarray.util.zeros(tt_size) for _ in range(num_candidates)]

    global_conflict_mask = bitarray.util.zeros(tt_size)
    if current_tt_conflict:
        for c in current_tt_conflict:
            global_conflict_mask |= c
    ...
    for node in reversed(tree_stack):
        if node.right is None:
            ...
            if cache is not None and node.left in cache:
                left_tt = cache[node.left]
            else:
                left_tt = compute_tt(node.left, input_tt=base_input_tt, cache=cache)
    ...
    has_conflict = bitarray.util.zeros(num_candidates)
    allowed_error = int(tolerance * denom)

    for i in range(num_candidates):
        # Calculate error for current output only (not including previous outputs)
        current_diff = (b[i] ^ tt) & ~n[i]
        current_error = current_diff.count(1)
        
        # Calculate cumulative error: union of current output error and previous outputs' errors
        # This matches the final error rate calculation which uses diff_map |= (t ^ o) for all outputs
        if global_conflict_mask.any():
            # Combine current output error with previous outputs' errors (union)
            cumulative_diff = current_diff | global_conflict_mask
            cumulative_error = cumulative_diff.count(1)
        else:
            cumulative_error = current_error
        
        # Check if cumulative error exceeds tolerance
        if cumulative_error > allowed_error:
            has_conflict[i] = 1
        else:
            has_conflict[i] = 0

    completeness = bitarray.bitarray([not n[i].any() for i in range(num_candidates)])
    return has_conflict, completeness


def sequential_synthesis(
    roots,
    verbose=False,
    command='resyn2',
    title: str = "sequential",
    num_inputs: int | None = None,
):
    """
    ABC resyn2 round-trip. Pass ``num_inputs`` equal to the original circuit's primary-input
    count so ``write_aiger`` emits contiguous PIs 1..N; after ``read_aiger``, leaf ``var``
    indices stay 0..N-1 and match ``compute_input_tt(num_inputs)`` / the reference circuit.
    """
    temp_dir = temp_dir_init()
    sequential_filename = temp_dir + "/" + (title if title is not None else "")
    raw_filename = sequential_filename + "_raw"
    write_aiger(roots, raw_filename + ".aig", with_symbol_table=False, num_inputs=num_inputs)
    
    # Count original AND gates
    orig_num_ands = count_num_ands(roots)
    
    if command == 'deepsyn':
        while True:
            os.system(("%s %s \"source %s; &read %s; &deepsyn; &write -n %s;\"" %
                       (abc_path, "-c" if verbose else "-q", abc_rc_path, raw_filename + ".aig", sequential_filename + ".aig"))
                      + (" > NUL" if platform.system() == 'Windows' else " 2>/dev/null"))
        ...
        while True:
            # Use source command to load abc.rc before using resyn2
            # Change to bin directory to ensure abc.rc is found, or use absolute path
            cmd = ("%s %s \"source %s; read %s; resyn2; write %s;\"" %
                   (abc_path, "-c" if verbose else "-q", abc_rc_path, raw_filename + ".aig", sequential_filename + ".aig"))
            os.system(cmd + (" > NUL" if platform.system() == 'Windows' else " 2>/dev/null"))
            if os.path.exists(sequential_filename + ".aig"):
                break
            else:
                time.sleep(0.1)
        ...
        while True:
            os.system(("%s %s \"source %s; &read %s; &transtoch -M 1 -V 0; &write -n %s;\"" %
                       (abc_path, "-c" if verbose else "-q", abc_rc_path, raw_filename + ".aig", sequential_filename + ".aig"))
                      + (" > NUL" if platform.system() == 'Windows' else " 2>/dev/null"))
            if os.path.exists(sequential_filename + ".aig"):
                break
    ...
    seq_roots, _ = read_aiger(sequential_filename + ".aig")
    seq_num_ands = count_num_ands(seq_roots)
    os.unlink(sequential_filename + ".aag")
    os.unlink(sequential_filename + ".aig")
    
    # Return (seq_roots, info) where info[4] is the optimized AND count
    # info format: [orig_num_ands, seq_num_ands, ...] to match expected usage
    info = [orig_num_ands, seq_num_ands, 0, 0, seq_num_ands]  # info[4] = seq_num_ands
    return seq_roots, info
...
def checkER(roots_1, roots_2, num_inputs=None):
    """
    Error rate between two circuits over the same input space.

    Uses **input-pattern** (column) semantics: a minterm counts as wrong if **any**
    output differs from the reference at that pattern (OR across outputs), matching
    typical ABC / ``Different columns / Total columns`` style metrics.

    ``num_inputs`` is inferred from primary input variables in both circuits (max index + 1);
    pass ``num_inputs`` explicitly when inference is ambiguous (e.g. unused inputs).
    """
    if not isinstance(roots_1, list):
        roots_1 = [roots_1]
    if not isinstance(roots_2, list):
        roots_2 = [roots_2]

    def _infer_num_inputs(roots):
        d = get_inputs_rec(roots)
        pos = [v for v in d.keys() if v >= 0]
        return max(pos) + 1 if pos else 0

    n1 = _infer_num_inputs(roots_1)
    n2 = _infer_num_inputs(roots_2)
    n = num_inputs if num_inputs is not None else max(n1, n2)
    if n <= 0:
        raise ValueError(
            "checkER: num_inputs could not be inferred (no primary inputs found); "
            "pass num_inputs explicitly."
        )

    input_tt = compute_input_tt(n)
    tt1 = compute_tts(roots_1, input_tt=input_tt)
    tt2 = compute_tts(roots_2, input_tt=input_tt)
    if len(tt1) != len(tt2):
        raise ValueError(f"checkER: output count mismatch {len(tt1)} vs {len(tt2)}")

    mismatch_mask = bitarray.util.zeros(len(tt1[0]))
    for out1, out2 in zip(tt1, tt2):
        mismatch_mask |= out1 ^ out2

    total_patterns = len(mismatch_mask)
    mismatch_count = mismatch_mask.count()
    return mismatch_count / total_patterns if total_patterns > 0 else 0.0
...
def write_aiger(root: NodeWithInv | list[NodeWithInv], filename: str = None, with_symbol_table=True, num_inputs=None) -> str:
    ...
    if num_inputs is None:
        ...
        # When num_inputs is provided, ensure all inputs from 1 to num_inputs are accounted for
        if len(info['inputs'].keys()) > 0:
            max_used_input = max(info['inputs'].keys())
            if max_used_input > num_inputs:
                print("Warning: num_inputs is smaller than the actual number of inputs, use the latter instead")
                num_inputs = max_used_input
        # input ids from 1 to num_inputs (even if some are unused)
        info['node_id'] = num_inputs + 1
    outputs = []
    for r in root:
    ...
    # Determine which inputs to write and in what order
    if num_inputs is not None:
        # When num_inputs is provided, write ALL inputs from 1 to num_inputs in order
        # This ensures all inputs are preserved even if unused (dont care)
        inputs_to_write = list(range(1, num_inputs + 1))
        num_inputs_in_file = num_inputs
    else:
        # When num_inputs is not provided, only write inputs that are actually used
        inputs_to_write = sorted([i for i in info['inputs'].keys() if i != 0])
        num_inputs_in_file = len(inputs_to_write)
    
    aiger_str = "aag %d %d 0 %d %d\n" % \
                (info['node_id'], num_inputs_in_file, len(outputs), len(info['ands_detail']))
    ...
    if with_symbol_table:
        # Write input symbols in the same order as inputs
        for idx, input_id in enumerate(inputs_to_write):
            symbol = info['inputs'].get(input_id, None)
...
def count_num_ands(root: NodeWithInv | list | None, node_set: set = None):
    if node_set is None:
        node_set = set()
    # Some callers may accidentally pass read_aiger()'s full return value
    # (roots, info). Accept it defensively by using the first element.
