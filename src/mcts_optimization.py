    ...
    for i in range(num_playouts):
        ...
        for l in range(num_leaf_parallelizations):
            ...
            while len(largest_node_set) > 0:
                ...
                # expansion
                env_batch = [node.info['env'] for node in nodes[l]]
                action_masks = np.stack([e.action_masks[-1] for e in env_batch], axis=0)
                # IMPORTANT: intersect env masks with encoder's valid token mask for each env.num_inputs
                # to avoid selecting tokens in the unused middle range (dynamic vocab).
                for j, e in enumerate(env_batch):
                    valid_mask = self.encoder.get_valid_token_mask(e.num_inputs)
                    if valid_mask.shape[0] < action_masks.shape[1]:
                        padded = np.zeros(action_masks.shape[1], dtype=bool)
                        padded[: valid_mask.shape[0]] = valid_mask
                        valid_local = padded
                    else:
                        valid_local = valid_mask[: action_masks.shape[1]]
                    action_masks[j] = np.logical_and(action_masks[j], valid_local)
                policies, cache = self._batch_estimate_policy(env_batch, src_tokens, src_pos_enc, src_action_mask,
                                                              action_masks, cache)
        ...
        vs, rollout_success, cache, rollout_debug_infos = self._batch_estimate_v_value_via_simulation_kvcache(
            ...
            max_inference_seq_length, cache, num_leaf_parallelizations, return_debug=True)
        ...
        rollout_debug_grid = [rollout_debug_infos[l * num_envs:(l + 1) * num_envs] for l in range(num_leaf_parallelizations)]
        simulation_time = time.time() - simulation_time

        paths = [[[] for _ in range(num_envs)] for _ in range(num_leaf_parallelizations)]
        for l in range(num_leaf_parallelizations):
            for j, node in enumerate(nodes[l]):
                pre_sum_reward = sum_rewards[l][j]
                clamp_floor = None
                clamped = False
                # IMPORTANT:
                # Do NOT clamp purely because rollout_success=False.
                # That collapses most failed rollouts to the same value (-orig_aig_size baseline),
                # destroys ranking signal, and can even clamp successful-but-low-reward rollouts.
                # Keep raw rollout value unless it is below a hard lower bound.
                if orig_aigs_size is not None and (vs[l][j] < -orig_aigs_size[j] - sum_rewards[l][j]):
                    clamp_floor = -orig_aigs_size[j] - sum_rewards[l][j]
                    node.v = clamp_floor
                    clamped = True
                else:
                    node.v = vs[l][j]
                ...
                node.info['rollout_debug'] = rollout_debug_grid[l][j] if rollout_debug_grid else None
                node.info['value_clamped'] = clamped
                node.info['clamp_floor'] = clamp_floor
                node.info['pre_sum_reward'] = pre_sum_reward
        ...
        if self.verbose > 0:
            for l in range(num_leaf_parallelizations):
                ...
                # Extra debug: per path termination reason and why value was clamped (e.g. to -orig_aig_size).
                details = []
                for j, node in enumerate(nodes[l]):
                    d = node.info.get('rollout_debug', {}) if isinstance(node.info, dict) else {}
                    details.append({
                        'path': paths[l][j],
                        'sum_reward': float(sum_rewards[l][j]),
                        'raw_v': float(node.info.get('raw_value', 0.0)),
                        'pre_sum': float(node.info.get('pre_sum_reward', 0.0)),
                        'clamped': bool(node.info.get('value_clamped', False)),
                        'clamp_floor': node.info.get('clamp_floor', None),
                        'reason': d.get('reason', 'n/a'),
                        'steps_run': d.get('steps_run', None),
                        'rollout_reward_sum': d.get('rollout_reward_sum', None),
                        'final_valid_actions': d.get('final_valid_actions', None),
                        'hit_unfinished_penalty': d.get('hit_unfinished_penalty', None),
                    })
                print("  rollout_details:", details)
