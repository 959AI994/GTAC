...
public:
    ...
    void ReadNet(const std::string & fileName, bool inpMapVerilog = true);
    ...
    void SynthAIG();
    bool SynthAndMap(double maxDelay, bool isVerb = false);
    bool SynthAndMap_v2(double maxDelay, bool isVerb = false, ll selectComm = 1);
    void SynthAndMapPro(double maxDelay, bool isVerb, bool & fGenNewSubNodes);
    void Sweep();
    ...
    std::vector <abc::Abc_Obj_t * > GetIdentNode( abc::Abc_Obj_t * pNode );
...
private:
    ...
    std::unordered_map<ll, ll> newIdMap; // newIdMap[oriId] = newId
...
public:
    ...
    void SetLevel(ll levelNew) {level = levelNew;}

    ll GetNewId(ll oriId) const;
    void ReplaceSubCktPro(std::vector <ll> vLI, std::vector <ll> vLO, abc::Abc_Ntk_t * pSubNtk, std::vector <ll> vLO_ori);
    void UpdateNewIdMap();
    bool IsOriIdAll0(std::string mark);

    std::pair <ll, ll> GetConstId(bool isVerb = false);
    std::pair <ll, ll> CreateConst(bool isVerb = false);
    ll CreateOneConst(bool isConst0);
    void PrintNodeLev(std::vector <ll> & targIds);
    void CreateConstNodes(bool isVerb = false);
    std::vector <ll> FindPartialPro();
    void ApplyCompensation(int RealCom, ll truncBit);
    void ProcVerTrun(std::vector <std::pair <ll,double>> & VetTruns);
    void MergeConst();
    void ReArrInTopoOrd();
    ...
    std::set <ll> GetPartialTFI(abc::Abc_Obj_t * pObj, ll TFI_Lev) const;
    void GetPartialTFIRec(abc::Abc_Obj_t * pObj, std::set<ll> & nodes, ll curLevel, ll maxLevel) const;
    ...
    void CalcTFO(abc::Abc_Obj_t * pObj, ll & coneSize, ll & nAddNum, bool fModifyMark) const;
    void CalcTFORec(abc::Abc_Obj_t * pObj, ll & coneSize, ll & nAddNum, bool fModifyMark) const;

    void Sweep();
    void ConvToSopWithTwoInps();
    void SynthWithResyn2Comm();
    void SynthAIG();
    // void SynthAndMapForSCL(bool isVerb = false);
    // void SynthAndMapForLUT();
    bool SynthAndMap(double maxDelay, bool isVerb = false);
    bool SynthAndMap_v2(double maxDelay, bool isVerb = false);
    void Print(bool showFunct = false) const;
    void PrintPro(bool showFunct = false, bool showFanouts = false, bool showNode2Po = false) const;
    void PrintLocal(bool showFunct = false, bool showFanouts = false, ll startId = 0, ll endId = 0) const;
    void Print_v2(bool showFunct = false, bool showFanouts = false, bool showNode2Po = false, int metrType = 0, ll nFrame = 100032, double errUppBound = 0.0) const;
    void PrintObjBas(abc::Abc_Obj_t * pObj, std::string && endWith) const;
    void PrintObj(abc::Abc_Obj_t * pObj, bool showFunct = false) const;
    void PrintObjPro(abc::Abc_Obj_t * pObj, bool showFunct = false, bool showFanouts = false) const;
    void PrintObj_v2(abc::Abc_Obj_t * pObj, bool showFunct = false, bool showFanouts = false, int metrType = 0, ll nFrame = 100032, double errUppBound = 0.0) const;
    ...
    void AddInv(ll & targId);
    void Trunc(ll truncBit);
    void SetBit(ll iBit, bool useConst1);
    void SetBitNode(ll iBit, bool useConst1, ll faninNum);
    void InvConstNode(bool isConst0, ll fanoutNum);
    void InvConstNodeAll(unsigned seed);
    void MergeIdentNode();
    bool CleanUp();
    ll GetMaxLev();
    void BreakModiGateAll();
    void BreakModiGate(ll & targetId);
    void FixConstInp(ll & constId, ll & targetId);
    bool ProcConstInp();
    std::pair <ll, ll> HasDoubleInv();
    bool ProcDoubleInv();
    ...
    abc::Abc_Obj_t * CreateGate2(std::vector <abc::Abc_Obj_t *> && fanins, const std::string & gateName, const std::string & PoName);
    
    void DumpCFile(std::string&& fileName);
    
    inline abc::Abc_Ntk_t * GetNet() const {return pNtk;}
    inline char * GetNetName() const {return GetNet()->pName;}
    ...
    inline ll GetOriId(abc::Abc_Obj_t * pObj) const {return abc::Abc_ObjGetOriId(pObj);}
    inline ll GetOriId(ll id) const {return abc::Abc_ObjGetOriId(GetObj(id));}
    inline void SetOriId(ll id, ll oriId) {abc::Abc_ObjSetOriId(GetObj(id), oriId);}
    inline void SetOriId(abc::Abc_Obj_t * pObj, ll oriId) {abc::Abc_ObjSetOriId(pObj, oriId);}
    void SetAllOriIdAsId();

    inline double GetMarkValue(abc::Abc_Obj_t * pObj) const {return abc::Abc_ObjGetMarkValue(pObj);}
    inline double GetMarkValue(ll id) const {return abc::Abc_ObjGetMarkValue(GetObj(id));}
    inline void SetMarkValue(abc::Abc_Obj_t * pObj, double value) {abc::Abc_ObjSetMarkValue(pObj, value);}
    inline void SetMarkValue(ll id, double value) {abc::Abc_ObjSetMarkValue(GetObj(id), value);}
    ...
    inline bool IsInv(ll id) const {return IsNode(GetObj(id)) && abc::Abc_NodeIsInv(GetObj(id));}
    ...
    inline void ReplaceObj(abc::Abc_Obj_t * pTS, abc::Abc_Obj_t * pSS) {abc::Abc_ObjReplace(pTS, pSS);}
    inline void ReplaceObj(ll tsId, ll ssId) {Replace(GetObj(tsId), GetObj(ssId));}
    ...
    inline void PrintObjPro(ll id, bool showFunct = false, bool showFanouts = false) const {PrintObjPro(GetObj(id), showFunct, showFanouts);}
    inline void PrintObj_v2(ll id, bool showFunct = false, bool showFanouts = false, int metrType = 0, ll nFrame = 100032, double errUppBound = 0.0) const {PrintObj_v2(GetObj(id), showFunct, showFanouts, metrType, nFrame, errUppBound);}

    inline bool CheckSCLNet() const {return AbcMan::CheckSCLNet(GetNet());}

    ll CalcMultiNodesMffcNum(std::vector<ll> nodesId);
    double CalcMultiNodesMffcArea(std::vector<ll> nodesId);
    void Abc_NtkCleanMarkD();
    void Abc_NtkCleanMarkDE();
    void CleanTravIds();
    double GetNodeArea(abc::Abc_Obj_t * pNode);
    void CheckDivisorNumForDisjNodes();
    bool CheckDisjointness(boost::dynamic_bitset <ull> poMark1, boost::dynamic_bitset <ull> poMark2);
    double CalcMarkArea();
    bool IsPathExist3(ll id1, ll id2, ll id3);
    bool IsPathExist2(ll id1, ll id2);
    bool SearchTFORec(ll targId, abc::Abc_Obj_t * pObj) const;
    bool SearchTFIRec(ll targId, abc::Abc_Obj_t * pObj) const;
    double GetInvArea() const;
    void ReplaceSubCkt(std::vector <ll> vDiv, std::vector <ll> vLO, abc::Abc_Ntk_t * pSubNtk, std::vector <ll> LoIndex);
    bool IsAllNodeMarkA1() const;
};
bool AreAllVectorElements0s(std::vector <int> vec);
void Abc_MfsWinSweepLeafTfo_rec_Pro(abc::Abc_Obj_t * pObj);
void PrintNtk(abc::Abc_Ntk_t * pNtk);
bool CheckSCLNet(abc::Abc_Ntk_t * pNtk);
// double GetLocalDelay(abc::Abc_Ntk_t * pNtk, std::vector <ll> vLO, std::vector <ll> vLI);
...
ll CountIntersection(const std::set<ll>& A, const std::set<ll>& B);