...
struct Abc_Obj_t_     // 48/72 bytes (32-bits/64-bits)
{
    ...
    // FOR ALS: (add by wenhui)
    int               oriId;         // original ID
    double            markValue;
    int               error0;
    int               error1;
    double            error0f;
    double            error1f;
    unsigned          fUseSmallFrame0  :  1;
    unsigned          fUseSmallFrame1  :  1;
    unsigned          fEarlyTerm0      :  1;
    unsigned          fEarlyTerm1      :  1;
    int               numof0s;
    int               numof1s;
    int               sngId;
    int               order;
    int               constType; // default: 0; const1: 1; const0: 2
    double            errInc;   // smallest error increase for this target node
    ...
    unsigned          fMarkD  :  1;  // add by wenhui
    unsigned          fMarkE  :  1;  // add by wenhui
    unsigned          fMarkF  :  1;  // add by wenhui
...
// read and set oriId (FOR ALS)
static inline int         Abc_ObjGetOriId( Abc_Obj_t * pObj )                    { return pObj->oriId;      }
static inline void        Abc_ObjSetOriId( Abc_Obj_t * pObj, int oriId )         { pObj->oriId =  oriId;    }
static inline double      Abc_ObjGetMarkValue( Abc_Obj_t * pObj )                { return pObj->markValue;  }
static inline void        Abc_ObjSetMarkValue( Abc_Obj_t * pObj, double value )   { pObj->markValue = value; }
