...
///                BY WENHUI                   ///
//////////////////////////////////////////////////
static inline void Scl_LibPinArrivalPro( SC_Timing * pTime, SC_Pair * pArrIn, SC_Pair * pSlewIn, SC_Pair * pLoad, SC_Pair * pArrOut, SC_Pair * pSlewOut, int fPrint )
{
    if (pTime->tsense == sc_ts_Pos || pTime->tsense == sc_ts_Non)
    {
        pArrOut->rise  = Abc_MaxFloat( pArrOut->rise,  pArrIn->rise + Scl_LibLookup(&pTime->pCellRise,  pSlewIn->rise, pLoad->rise) );
        ...
        pSlewOut->fall = Abc_MaxFloat( pSlewOut->fall,                Scl_LibLookup(&pTime->pFallTrans, pSlewIn->fall, pLoad->fall) );
        if (fPrint) {
            printf("d_rise(pos) = %.4f; pArrOut->rise = %.4f \n", Scl_LibLookup(&pTime->pCellRise,  pSlewIn->rise, pLoad->rise), pArrOut->rise);
            printf("d_fall(pos) = %.4f; pArrOut->fall = %.4f \n", Scl_LibLookup(&pTime->pCellFall,  pSlewIn->fall, pLoad->fall), pArrOut->fall);
            printf("S_rise(pos) = %.4f; pSlewOut->rise = %.4f \n", Scl_LibLookup(&pTime->pRiseTrans, pSlewIn->rise, pLoad->rise), pSlewOut->rise);
            printf("S_fall(pos) = %.4f; pSlewOut->fall = %.4f \n", Scl_LibLookup(&pTime->pFallTrans, pSlewIn->fall, pLoad->fall), pSlewOut->fall);
        }
    }
    if (pTime->tsense == sc_ts_Neg || pTime->tsense == sc_ts_Non)
    {
        ...
        pSlewOut->fall = Abc_MaxFloat( pSlewOut->fall,                Scl_LibLookup(&pTime->pFallTrans, pSlewIn->rise, pLoad->fall) );
        if (fPrint) {
            printf("d_rise(neg) = %.4f; pArrOut->rise = %.4f \n", Scl_LibLookup(&pTime->pCellRise,  pSlewIn->fall, pLoad->rise), pArrOut->rise);
            printf("d_fall(neg) = %.4f; pArrOut->fall = %.4f \n", Scl_LibLookup(&pTime->pCellFall,  pSlewIn->rise, pLoad->fall), pArrOut->fall);
            printf("S_rise(neg) = %.4f; pSlewOut->rise = %.4f \n", Scl_LibLookup(&pTime->pRiseTrans, pSlewIn->fall, pLoad->rise), pSlewOut->rise);
            printf("S_fall(neg) = %.4f; pSlewOut->fall = %.4f \n", Scl_LibLookup(&pTime->pFallTrans, pSlewIn->rise, pLoad->fall), pSlewOut->fall);
        }
    }
    if (fPrint)
        printf("\n");
