...
static inline void Abc_SclTimeFaninPro( SC_Man * p, SC_Timing * pTime, Abc_Obj_t * pObj, Abc_Obj_t * pFanin, int k, int fPrint )
{
    SC_Pair * pArrIn   = Abc_SclObjTime( p, pFanin );
    SC_Pair * pSlewIn  = Abc_SclObjSlew( p, pFanin );
    SC_Pair * pLoad    = Abc_SclObjLoad( p, pObj );
    ...
    if ( p->pFuncFanin ) pLoad->fall += p->pFuncFanin(p, pObj, pFanin, k, 0);
    if ( p->pFuncFanin ) pLoad->rise += p->pFuncFanin(p, pObj, pFanin, k, 1);
    Scl_LibPinArrivalPro( pTime, pArrIn, pSlewIn, pLoad, pArrOut, pSlewOut, fPrint );
}


void Abc_SclTimeNodePro( SC_Man * p, Abc_Obj_t * pObj, int fDept, int fPrint )
{
    SC_Timing * pTime;
    SC_Cell * pCell;
    int k;
    ...
    Abc_Obj_t * pFanin;
    if (fPrint) {
        printf("n%d ", Abc_ObjId(pObj));
        SC_Cell * pCell = Abc_ObjIsNode(pObj) ? Abc_SclObjCell(pObj) : NULL;
        printf( "%-*s:\n", 25, pCell ? pCell->pName : "PI" );
    }
    if ( Abc_ObjIsCi(pObj) )
    {
        assert( !fDept );
...
//    if ( !(Abc_ObjFaninNum(pObj) == 1 && Abc_ObjIsPi(Abc_ObjFanin0(pObj))) && p->EstLoadMax && Value > 1 )
    ...
    Abc_ObjForEachFanin( pObj, pFanin, k )
    {
        printf("fanin n%d:", Abc_ObjId(pFanin));
        pTime = Scl_CellPinTime( pCell, k );

        SC_Pair * pArrIn   = Abc_SclObjTime( p, pFanin );
        SC_Pair * pSlewIn  = Abc_SclObjSlew( p, pFanin );
        SC_Pair * pLoad    = Abc_SclObjLoad( p, pObj );
        printf("pArrIn->rise = %.4f, pArrIn->fall = %.4f; ", pArrIn->rise, pArrIn->fall);
        printf("pSlewIn->rise = %.4f, pSlewIn->fall = %.4f; ", pSlewIn->rise, pSlewIn->fall);
        printf("pLoad->rise = %.4f, pLoad->fall = %.4f \n", pLoad->rise, pLoad->fall);
        
        if ( fDept )
            Abc_SclDeptFanin( p, pTime, pObj, pFanin, k );
        else
            Abc_SclTimeFaninPro( p, pTime, pObj, pFanin, k, fPrint );
    }
    if ( p->EstLoadMax && Value > 1 )
    {
        pLoad->rise = LoadRise;
...
void Abc_SclTimeNtkRecomputePro( SC_Man * p, float * pArea, float * pDelay, int fReverse, float DUser, int fPrint )
{
    Abc_Obj_t * pObj;
    float D;
    int i;
    ...
    p->nEstNodes = 0;
    Abc_NtkForEachCi( p->pNtk, pObj, i )
        Abc_SclTimeNodePro( p, pObj, 0, fPrint );
    Abc_NtkForEachNode1( p->pNtk, pObj, i )
        Abc_SclTimeNodePro( p, pObj, 0, fPrint );
    Abc_NtkForEachCo( p->pNtk, pObj, i )
        Abc_SclTimeNodePro( p, pObj, 0, fPrint );
...
SC_Man * Abc_SclManStartPro( SC_Lib * pLib, Abc_Ntk_t * pNtk, int fUseWireLoads, int fDept, float DUser, int nTreeCRatio, int fPrint )
{
    SC_Man * p = Abc_SclManAlloc( pLib, pNtk );
    if ( nTreeCRatio )
    {
    ...
    Abc_SclTimeNtkRecomputePro( p, &p->SumArea0, &p->MaxDelay0, fDept, DUser, fPrint );
    p->SumArea  = p->SumArea0;
    p->MaxDelay = p->MaxDelay0;
    return p;
}
...
{
    SC_Man * p;
    p = Abc_SclManStartPro( pLib, pNtk, fUseWireLoads, 1, 0, nTreeCRatio, fPrint );
    Abc_SclTimeNtkPrint( p, fShowAll, fPrintPath );
    if ( fDumpStats )
        Abc_SclDumpStats( p, "stats.txt", 0 );
    Abc_SclManFree( p );
